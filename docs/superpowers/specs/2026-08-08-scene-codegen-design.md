...
> 2026-08-08 初稿，同日经两轮自我挑刺后重写。挑出来的问题与改法记在最后一节
> 「自我挑刺记录」：**4 处内部矛盾、8 处遗漏、7 条交互不足**。
...
拿三道**已经有动画**的题，用新流程重跑一遍（新 sid、不落库、不碰现有场景）：

| 新 | 源 | 旧轮数 | 新轮数 | 新耗时 | 结果 |
|---|---|---|---|---|---|
| q4-cg2 | q4-gen3 | 1 | **1** | 1.6 分 | PASS |
| q1-cg1 | q1-gen2 | 1 | **1** | 13.5 分 | PASS |
| q13-cg1 | q13-gen3 | **3** | **1** | 13.7 分 | PASS |

三道全部**第一轮通过**，独立复跑 ⑥ 也都 PASS。agent 要写的 JS 从 392 行降到
209 行（**-47%**）。

### 达标的两项

**轮数降了。** q13-gen3 旧流程 3 轮，新流程 1 轮。L4 与 L1 一次都没失败过 ——
这不是运气，是结构性的（物理来自已验证的表，披露文字来自生成的 figcaption）。

**agent 写的东西少了一半。** 而且少掉的正是最容易错的那部分。

### **没达标的一项：单轮耗时**

q1-cg1 用了 13.5 分钟、q13-cg1 用了 13.7 分钟 —— **仍在旧流程的量级**。
只有最简单的 q4-cg2 快到 1.6 分钟。

这跟设计里那句「慢不在重试，在单轮本身」是一致的：**代码生成拿走的是物理，
而单轮时间大头是画图**，画图还是得模型一笔一笔想。所以

> **不要拿这次改动去宣传「⑤ 变快了」。** 它变的是「更少失败、更少返工、
> agent 写得更少」，复杂题的单轮时间基本没动。

真要压单轮时间，得往「连场景也声明式」那个方向走（本设计的非目标），
或者接受画图就是慢。**那是另一次改动，先别顺手做。**

## 验收：三项全部达标（2026-08-08 晚）

设计里把成败卡死在三个数上。**现在三项都有结论了。**

### ① 轮数：达标

实测三道 + 真实新卷子两道，**五道全部第一轮通过**。
其中 q13-gen3 旧流程 3 轮、新流程 1 轮。L4 与 L1 一次都没失败 —— 结构性的，不是运气。

### ② 单轮耗时：**没达标**（结论不变）

复杂题仍是 13 分钟量级。慢的主因是画图，而画图这次没动。
**别拿这次改动宣传「⑤ 变快了」。**

### ③ 人工目检：达标（老师看过）

真实卷子上线后老师的原话：「动画本身还是不错的，没有压字的现象了」
「时间轴也是挺好的」。

**而且测量印证了这个观察，比预期还好。**「每条文字压中几个图形」归一化：

| 组 | 数值 |
|---|---|
| 旧 · 已知乱（q14/q7/q19/q8-gen3/q6） | 1.79 |
| 旧 · 已知干净（q17/q11/q4-gen3/q1-gen2/q15） | 0.27 |
| **新 · 代码生成 5 个** | **0.13**（范围 0.00 – 0.27） |

**最差的一个正好等于旧「干净组」的平均值。** 版面问题是被结构性解决的 ——
面板由代码排，agent 不再在那块地方摆字，而面板恰恰是以前压字最狠的地方。

### 「铺开」不是一个待办

`build()` 里 `can_codegen(spec)` 是**无条件判**的，所以**每一份新卷子已经在走这条路**。
刚上传的那份（2025 黑龙江/吉林/辽宁/内蒙古卷）两道动画都是 `gen=codegen`，
就是证据。不存在一个要不要打开的开关。

现有 36 个老场景不动 —— 它们已经过门禁、已经在页面上跑。

### L6（文字压图形门禁）：现在可以上，但优先级降了

代码生成之后所有新场景都在 0.00–0.27，而阈值候选是 0.8 —— **余量很大**。
所以 L6 现在的价值从「抓现有问题」变成「防回归」：它永远不该触发，
一旦触发就说明有东西退化了。这种门禁很便宜，值得加，但不紧急。

**那个盲区仍然没补上**：这条判据只有排斥力没有吸引力。q13-cg1 测出 0.00，
是真干净还是「标签甩进空白处」，**没有人看过那张图**，不能只凭这个数就说它好。

## 后续方向的测量（2026-08-08，41 个场景）

上一轮结束时留了个问题：**要不要往「连场景也声明式」走**（本设计的非目标）。
跑了一次归类测量，结果**把两条候选路线分开了**。
...
| 场标记（×网格） | 3.4% |
| 折线/多边形 + 圆弧 + 椭圆 | 1.5% |
| **自由曲线（贝塞尔）** | **0.2%**（2118 个里只有 5 个） |

`path` 命令里连三次贝塞尔 `C` 一个都没有，只有 `M/L`(105)、`A` 圆弧(14)、`Q`(5)。
每个场景平均只用 5.6 种图元（最少 4、最多 7）。

### 场景原型：**不收敛**

41 个场景按构成分型，**分出 18 种，最大的一类才 5 个**，长尾里 11 种各只有 1-2 个。
**同一套图元反复用，但组合方式几乎每题一个样。**

### 结论：两条路线的判决相反

| 路线 | 判决 | 理由 |
|---|---|---|
| **模板库**（挑模板 + 填参数） | **不成立** | 41 个场景 18 种组合，等于每 2 道题一个模板，新卷子还会不断冒出新组合 |
| **声明式图元 + 排版引擎**（④ 顺带产出场景描述，⑤ 退化为纯渲染） | **成立** | 它不要求场景能归类，只要求图元有限 —— 而图元确实有限 |

**这里更正一次判断**：做这次改动时我以为「两条路线由同一个测量决定」。
测下来它们的判决**相反**，捆在一起想是错的。

### 一个之前没看到的协同

**文字标签占全部图元的 47.5%**，将近一半。而标签正是 L5 版面问题的来源
（压字、把标签甩进空白处），也是老师直接抱怨的「文字重叠」。

**一个会自动摆标签的排版引擎，同时解决「慢」和「乱」两件事。**
而且因为它是确定性的，可以让它**自己保证不压字**（摆标签时查碰撞、自动避让），
不需要靠事后门禁抓 —— 这也顺带绕开了「④ 写场景描述时没有反馈回路」那个难点。

**下一轮要做的话，重心是排版引擎，不是图元库。** 图元这关已经过了。
