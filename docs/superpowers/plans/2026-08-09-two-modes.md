...
def test_解析试卷仍然跑原来那三段():
    got = [label for label, _ in api.resume_steps_for("pdf")]
    assert got[0].startswith("②d")
    assert any("解题" in s for s in got)


def test_答题卡只跑读答案和知识点():
    got = [mod for _, mod in api.resume_steps_for("answers_only")]
    assert got == ["refread.py", "kpmark.py"]


def test_答题卡不许出现解题断言动画():
    got = " ".join(m for _, m in api.resume_steps_for("answers_only"))
    for forbidden in ("solve.py", "spec.py", "scene.py", "assemble.py"):
...
# 解析试卷那三段（②d / ③ / 收尾）是特殊的：③ 和收尾各自还有子步骤，
# 所以下面用 None 标出来，由 resume_paper 走原来那两个函数
_RESUME = {
    "paper": [("②d 标准答案", "refans.py"), ("③ 解题", None), ("收尾", None)],
    "sheet": [("Ⓐ 读参考答案", "refread.py"), ("③c 知识点", "kpmark.py")],
}


def resume_steps_for(source_kind):
    """这个模式续跑要走哪几步。返回 [(显示名, 脚本名或 None)]。"""
...
def resume_paper(jid, name, source_kind=None):
    ...
    **按模式走。** 不分模式的话，对一份答案卷点下去会去跑 solve/spec/scene ——
    ...
    try:
        ...
        for label, mod in resume_steps_for(source_kind):
            if mod == "refans.py":
                run_step(jid, label, step_path(mod) + [name], timeout=120)
            elif mod == "refread.py":
                # 续跑不重新读图（原件早收掉了）。这一步只有在「卷子建了但一题
                # 都没有」时才有意义，而那种情况该重传，不该在这里假装能接上
                job_log(jid, "   Ⓐ 已经读过了，跳过 —— 要重读请重新上传参考答案")
            elif mod == "kpmark.py":
                run_step(jid, label, step_path(mod) + [name], timeout=1800)
...
```css
/* 两个模式的 tab。选中的那个用实底，没选中的只有文字 ——
   它们是两件事，视觉上要比一个下拉筛选器重 */
.modes { display: flex; gap: 4px; margin: 0 0 18px; border-bottom: 1px solid #e5e7eb; }
.modes button {
  background: none; border: 0; border-bottom: 2px solid transparent;
  padding: 8px 14px; font-size: 15px; color: #6b7280; cursor: pointer;
}
.modes button.on { color: #111827; border-bottom-color: #111827; font-weight: 600; }
.fieldrow { display: flex; align-items: center; gap: 10px; margin: 0 0 10px; }
.fieldrow > span { color: #6b7280; font-size: 14px; }
.fieldrow input {
...
export default function AnswerQuestionCard({ q }: { q: Question }) {
  return (
    <section className="acard" id={`q${q.n}`} tabIndex={-1}>
      ...
      <dl>
        <dt>标准答案</dt>
        <dd className="ans"><MathText text={q.refAnswer ?? ''} math={[]} /></dd>

        <dt>官方解答</dt>
        <dd>
          {q.refSolution
            ? <MathText text={q.refSolution} math={[]} />
...
```css
.acard { border: 1px solid #e5e7eb; border-radius: 8px; padding: 14px 16px; margin: 0 0 12px; }
.acard h3 { margin: 0 0 10px; font-size: 15px; }
.acard dl { display: grid; grid-template-columns: 84px 1fr; gap: 6px 14px; margin: 0; }
.acard dt { color: #6b7280; font-size: 13px; }
.acard dd { margin: 0; }
.acard .ans { font-weight: 600; }
.acard .dim { color: #9ca3af; }
.acard .kp {
  display: inline-block; background: #f3f4f6; border-radius: 4px;
  padding: 2px 8px; margin: 0 6px 4px 0; font-size: 13px;
}
.acard .kp i { color: #9ca3af; font-style: normal; margin-left: 6px; }
```
...
```bash
# 答题卡库里不该出现任何 PDF 卷子，试卷库里不该出现任何答案卷
curl -s "http://127.0.0.1:8712/api/papers?mode=sheet" -b <你的会话cookie> | python3 -m json.tool | grep -c answers_only
```

更要紧的是在页面上做这一条：**切到解析试卷，随便打开一份跑完的卷子，确认六格标志、动画、解法一样不少。**
