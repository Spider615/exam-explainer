...
> **别在这里加 `store.progress(name)`。** 加了会给 `tests/test_progress.py::PaperApiFailureTests`
> 塞进一个它没有的库依赖：那条用例把 `get_paper` / `paper_solutions` / `assembled` 都 patch 了，
> 唯独没 patch `progress`。单独跑它会连不上测试库；全量跑却是绿的 —— 因为字母序更靠前的
> 某个文件先触发了 session 级 `db` fixture 建了库。**「只有整套跑才绿」是假绿**，正是这个
> 项目最不能接受的那种。
...
**这条命令必须能单独跑绿。** 它只挑了两个文件，不会触发别的文件带起来的建库 fixture ——
所以它同时是「有没有偷偷新增库依赖」的探针。红了就说明加了不该加的查询。

- [ ] **Step 6: 跑全量**

Run: `.venv/bin/python -m pytest tests/ -q`
Expected: PASS，465 passed
...
- Modify: `tests/test_modes.py`（接住那条被取代的跨语言门禁，见 Step 0）
- Delete: `tests/test_stage_code_covered.py`（见 Step 0）

> **Step 0 必须先做，否则这个任务一定红。** `tests/test_stage_code_covered.py` 是一条跨语言门禁：它去 `PaperView.tsx` 里挖 `STAGE_OF_CODE` 那张表，断言后端会返回的每个代号前端都认得。本任务把那张表删掉之后，它会在 `split("STAGE_OF_CODE...")` 那一步抛 `IndexError`。
>
> **它不是被删掉，是被取代了。** 它守的那条不变量（「代号 → 哪一格」不能漏）现在由 `tests/test_modes.py::test_stage_of会返回的每个代号都登记过` 在后端守着，而且守得更严 —— 前端已经没有第二张表可漏了。但它还守着**第二条**不变量：`api.py` 里 `run_pipeline` 的 `step_code`（①/② 挂掉时给的失败代号）必须落得进那排格子。**这一条不能丢**，要搬进 `test_modes.py`。

**Interfaces:**
- Consumes: Task 3 下发的 `mode.stages`
- Produces: 无（纯前端）

- [ ] **Step 0: 把那条被取代的门禁接住，再删掉它**

先追加到 `tests/test_modes.py` 末尾：
...
跑一次确认它绿：`.venv/bin/python -m pytest tests/test_modes.py -q`

然后删掉被取代的那条，并**在同一个 commit 里**说清楚为什么：

```bash
git rm tests/test_stage_code_covered.py
```
...
```bash
git add web/src/types.ts web/src/components/PaperView.tsx tests/test_modes.py
git rm --cached -q tests/test_stage_code_covered.py 2>/dev/null || true
git add -A tests/
git commit -m "$(cat <<'MSG'
...
test_stage_code_covered.py 跟着删：它是一条跨语言门禁，去 PaperView.tsx 里
挖 STAGE_OF_CODE 那张表比对。表没了它就没有对象可查。**它不是被删，是被
取代了** —— 它守的第一条不变量现在由 test_modes 在后端守着且更严，
第二条（①/② 的失败代号必须落得进格子）已经搬进 test_modes。
...
**Files:**
- Modify: `pipeline/store.py`（`get_paper` 把 `layout` 缺的键补齐，见 Step 0）
...
- Create: `tests/test_answer_paper_opens.py`（真库，不打桩，见 Step 0）

> **Step 0 必须先做。** 2026-08-09 实测：`GET /api/papers/<答案卷>` **一直是 500**。
> `store.get_paper` 里 `q.update(q.pop("layout") or {})` —— 答案卷的 `layout` 是
> NULL，于是 `figures` / `fig_marks` 这两个键**根本不存在**，而 `api.paper` 那句
> `"figures": [fig(f) for f in x["figures"]]` 是硬取，当场 `KeyError`。
>
> 也就是说**答题卡详情页从来就打不开**，而 Task 11 正要建在这个端点上。
>
> 这个洞躲过了所有测试，原因很具体：这个端点现有的测试**全是打桩的**，
> 而打桩的 `get_paper` 返回值是人手写的、键是齐的。**所以 Step 0 的测试必须走真库。**

**Interfaces:**
- Consumes: `store.get_paper` 已经查出来的 `ref_solution`
- Produces: `GET /api/papers/{name}` 每道题多一个键 `refSolution: string | null`

- [ ] **Step 0a: 写一条走真库的失败测试** —— 新建 `tests/test_answer_paper_opens.py`
...
Run: `.venv/bin/python -m pytest tests/test_answer_paper_opens.py -q`
Expected: FAIL —— `KeyError: 'figures'`

- [ ] **Step 0c: 在 `store.get_paper` 里把形状补齐**

`pipeline/store.py`，把 `q.update(q.pop("layout") or {})` 那一段改成：
...
Run: `.venv/bin/python -m pytest tests/ -q`
Expected: PASS，469 passed（463 基线 + Step 0 的 3 条 + Step 1 的 3 条）

- [ ] **Step 7: 提交**

```bash
git add pipeline/store.py pipeline/api.py web/src/types.ts \
        tests/test_ref_solution_reaches_api.py tests/test_answer_paper_opens.py
git commit -m "$(cat <<'MSG'
fix: 答案卷的详情页打不开，以及官方解答过程到不了页面

两个洞叠在一条链上，都是「后端做完了但页面拿不到」：

1. GET /api/papers/<答案卷> 一直是 500。答案卷的 layout 是 NULL，
   figures/fig_marks 两个键根本不存在，而 api.paper 那句 x["figures"]
   是硬取。也就是说答题卡详情页从来就打不开。
   它躲过了所有测试，因为这个端点现有的测试全是打桩的，而人手写的桩
   键是齐的 —— 所以补的这条走真库。
2. get_paper 查了 ref_solution，投影里漏了。官方解答过程是「答题卡诊断」
   承诺的核心产出，它在库里躺着没人看得见。
...
### Task 13: ③c 在答题卡这条链上不能空跑

> **这个任务是端到端实跑之后加的。** 2026-08-09 实跑：上传入口跑通、26 题入库、
> 真值表 4/4，但 **③c 挂上 0 题、26 题一个都没挂上**。
>
> 原因确定：`kpmark.payload_for()`（`kpmark.py:75`）只读 `stem` 和 ③ 的解法，
> **根本不读 `ref_answer` / `ref_solution`**。答题卡这条链两样都没有，于是送给模型的
> 26 段全是「【第N题】/ 题干：/ 解法：（尚未解出）」—— 一片空白，挂不上是必然的。
>
> 后果：答题卡模式的终点就是 ③c，③c 空跑 → 每份答案卷**永远停在
> 「③c 知识点 0/26 · 已停止」，永远到不了「已完成」**，页面上 26 行「没挂上知识点」。
>
> 这正是设计文档里那条：「`kpmark` 现在的输入是「题干 + ③ 的解法」。这条链改成
> 「题干（Ⓔ）+ 官方解答（Ⓐ）」，**两样有一样就能挂**。其余一个字不改 —— 词表、
> `keep()` 的过滤规则、最多 3 个、挂不上留空，全部照旧。」

**Files:**
- Modify: `pipeline/kpmark.py:75-88`（`payload_for`）
- Create: `tests/test_kpmark_payload.py`

**Interfaces:**
- Consumes: `store.get_paper(name)` 返回的 `sourceKind`、每题的 `ref_answer` / `ref_solution`
- Produces: `kpmark.payload_for(paper, sols) -> str`（签名不变）

- [ ] **Step 1: 写失败的测试** —— 新建 `tests/test_kpmark_payload.py`
...
Run: `.venv/bin/python -m pytest tests/test_kpmark_payload.py -q`
Expected: FAIL —— 答题卡那几条红（`2BIL / MP` 不在输出里、`尚未解出` 反而在）
...
Run: `.venv/bin/python -m pytest tests/test_kpmark_payload.py tests/test_kpmark.py -q`
Expected: PASS。`tests/test_kpmark.py` **一行不许改**，它不改还绿才是解析试卷那条链没被碰坏的凭据。

- [ ] **Step 5: 跑全量**

Run: `.venv/bin/python -m pytest tests/ -q`
Expected: PASS，508 passed（502 + 6）

- [ ] **Step 6: 提交**

```bash
git add pipeline/kpmark.py tests/test_kpmark_payload.py
git commit -m "$(cat <<'MSG'
fix: ③c 在答题卡那条链上是空跑

端到端实跑抓到的：26 题一个都没挂上知识点。payload_for 只读题干和 ③ 的
解法，而答题卡这条链两样都没有 —— 送过去的是 26 段只有题号的空白，
模型挂不上是必然的。而标准答案和官方解答过程就在手边，只是没人读。

后果不只是「少了标签」：③c 是这个模式的终点，空跑意味着每份答案卷永远
停在「0/26 · 已停止」，永远到不了「已完成」。
