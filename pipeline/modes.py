...
def _stage_of_sheet(pg):
    ...
    **不分支的话 solutions/specs/scenes 恒为 0，进度带永远转、done 永远是 false。**
    ...
    ★ 学生的答题卡（Ⓢ 抠图 / Ⓑ 读批改 / Ⓒ 判定）**不在这里，也不占格子**
    ------------------------------------------------------------------
    这一条是「进度永远走不到头」那次事故的镜像版，**两个方向都会撒谎**，
    所以两边都要写死：

      · 加格子 → 一份**没传答题卡**的答案卷永远走不到 `done`，整批退回
        「未完成」。连锁还不止显示：`failure_note` 会复活旧失败、
        `api.resume_gate` 的 done 闸失效。而答题卡是**选填**的。
      · 不加格子、也不给别的出口 → Ⓑ 跑十分钟，这份卷子一直写着「已完成」。

    取的是「不加格子 + 另给出口」：答题卡的进度、失败、busy 走
    `GET /api/papers/{name}` 里那份 `sheets`（`store.list_sheets`），
    页面按**卡**画，一卡一条进度。

    不占格的理由和 `stemread`（Ⓔ 读题干）不占格同源，见下面 `SHEET.cell_of`
    的注释：选填的东西给它一格，要么永远灰着（读作卡住了）、要么打勾
    （撒谎说读过了）。

    卷子这一级的 `done` 说的是**参考答案侧**跑完没有，和答题卡读到哪无关 ——
    所以这个函数一个 sheet 键都不看。真要看也必须 `pg.get`：
    `tests/test_stage_answers_only.py` 的 BASE 里没有那些键，硬取会 KeyError，
    让 `/api/papers` 整个 500。
    """
    q = pg["questions"]
    if not q:
