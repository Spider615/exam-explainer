...
export interface Progress {
  ...
  /**
   * 在跑的那个活干的是**哪份答题卡**；不是答题卡那条链就是 null。
   *
   * 拿它决定进度卡的标题听谁的。`stage` 是从库里**推断**出来的卷子级阶段，
   * 而推断那一套（`_stage_of_sheet`）**有意不看答题卡** —— 给它加一格的话，
   * 没传过答题卡的卷子永远走不到「已完成」。于是 Ⓢ/Ⓑ/Ⓒ 在跑的时候，
   * `stage` 说的是**另一件事**：实测显示成「③c 知识点 0/26 · 已用时 32 分」，
   * 而知识点那一步根本没在跑。
   *
   * **不靠认 `step` 里的字符串**：任务建出来那一刻就写了 `sheet=<卡 id>`。
   */
  sheet?: number | null
}
