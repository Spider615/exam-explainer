var PERIOD = 4.0;
var READOUTS = ["t", "v1", "a1", "v2", "a2", "QR", "v1_post", "v2_post"];
...
function drawFrame(ps, u, svg) {
  ...
  setLine(svg.querySelector('#q13-lo-ab'), pt(abC, NRM, -18), pt(abC, NRM, 18));
  svg.querySelector('#q13-lo-ab-lab').setAttribute('x', abC.x);
  svg.querySelector('#q13-lo-ab-lab').setAttribute('y', abC.y - 12);

  setLine(svg.querySelector('#q13-lo-cd'), pt(cdC, NRM, -18), pt(cdC, NRM, 18));
  svg.querySelector('#q13-lo-cd-lab').setAttribute('x', cdC.x);
  svg.querySelector('#q13-lo-cd-lab').setAttribute('y', cdC.y + 20);

  // F 箭头：碰前作用于cd，碰撞后撤去
  var fArrow = svg.querySelector('#q13-lo-F-arrow');
  var fLab = svg.querySelector('#q13-lo-F-lab');
  if (collided) {
  ...
  // v1 v2 速度箭头
  var v1Arrow = svg.querySelector('#q13-lo-v1-arrow');
  var v2Arrow = svg.querySelector('#q13-lo-v2-arrow');
  if (!collided) {
  ...
  // 碰撞闪光与碰后速度箭头
  var flash = svg.querySelector('#q13-lo-flash');
  var v1post = svg.querySelector('#q13-lo-v1post-arrow');
  var v2post = svg.querySelector('#q13-lo-v2post-arrow');
  if (collided) {
  ...
  // QR 进度条
  var bar = svg.querySelector('#q13-lo-QR-bar');
