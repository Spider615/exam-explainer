...
这是一条**踩出来的**规则。L5 只问「两条文字有没有压在一起」，它不问标签离
它标注的东西有多远。于是消除重叠最省事的办法就是把标签扔进空白处 ——
门禁绿了，而图废了：读者不知道哪个标签说的是哪根杆、哪个电阻。

实测有一次重跑就是这么"修"的：`ab（m₁=0.8kg，接R）` 从紧挨着 ab 棒被挪到
左上角空白，`30°` 角标从斜面底端被挪开、不再标注任何东西，整张图缩在中间、
文字散在四周。一个字都没删，门禁全绿，但比返工前差得多。

所以：

- **对象标签（`ab` `cd` `R=0.02Ω` `30°` 这类）必须紧挨它标注的那个图形元素。**
  实在挤不下，就画一条细引出线（`class="sh"`）把标签和对象连起来 ——
  **不许让读者自己猜哪个标签说的是哪个东西。**
- 角度标记要连在构成那个角的两条线上，不能单独浮在一边。
- 消除重叠的正确手段只有三个：**整块平移、加高画布、把长说明搬进 figcaption**。
  把单个标签甩进远处空白不在其列。
- 读数面板、标题这类**成块的说明文字**不受这条约束 —— 它们本来就该在自己的
  区域里，离图形远是正常的。这条针对的是**指向具体对象的标签**。
